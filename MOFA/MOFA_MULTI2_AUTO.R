...
print(paste("UPLOADING", type, i, sep=" "))
...
for (iter in 3:factors){

n_factors = iter

dir.create(path = paste(directory,"/MOFA/OUTPUT/","MOFA", "_", args[1] ,"_vs_", args[2], "_",as.numeric(n_factors),"_factors" ,sep="") ,  showWarnings = TRUE, recursive = TRUE, mode = "0777")

directory2 <- paste(directory,"/MOFA/OUTPUT/","MOFA", "_", args[1] ,"_vs_", args[2], "_",as.numeric(n_factors),"_factors" ,sep="") 
setwd(directory2)
...
pdf(file= paste("MOFA", "_", args[1] ,"_vs_", args[2], "_1",".pdf", sep =""), width = 20, height = 9)
...
line = model@cache$variance_explained$r2_total[[1]]
write.table(line,file=paste("MOFA_explained_tot_",n_factors,".tsv", sep =""))

line = model@cache$variance_explained$r2_per_factor[[1]]
write.table(line,file=paste("MOFA_explained_factors_",n_factors,".tsv", sep =""))

line = model@cache$variance_explained$r2_per_factor[[1]]
line<-round(line,2)
total<-apply(line,2,sum)
total<-rbind(line,total)
write.table(total,file=paste("MOFA_explained_factors_rounded_",n_factors,".tsv", sep =""))
...
if (n_factors <= COMMAND_MOFA[4,2]){
        focus_factor <- 1
}else{
        focus_factor <- COMMAND_MOFA[4,2]
}
...
best<-length(nice)
if (length(nice) != 0) {
best2 <- sum(D[which(D$p.adj < 0.05),][,2])
}else{best2 <- 100}
...
write.table(best_of_you,file= paste(directory,"/MOFA/OUTPUT/","AUTOMATIC_SEARCH_BEST_FACTORS", "_", args[1] ,"_vs_", args[2] ,".tsv",sep=""),quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
line="#ONLY THE TOP 3 MOFA MODELS ARE RETAINED#"
write(line,file= paste(directory,"/MOFA/OUTPUT/","AUTOMATIC_SEARCH_BEST_FACTORS", "_", args[1] ,"_vs_", args[2],".tsv" ,sep=""),append=TRUE)
...
if (dir.exists(paste(directory,"/MOFA/OUTPUT/",to, sep=""))) {
        unlink(paste(directory,"/MOFA/OUTPUT/",to, sep=""), recursive = TRUE)
}}
